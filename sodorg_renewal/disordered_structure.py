...
class DisorderGroup:
    ...
    def _process_occupancies(self) -> None:
        ...
        if self.occupancy:
            if not self.atoms.has('occupancies'):
                self.atoms.new_array('occupancies', np.ones(len(self.atoms)) * self.occupancy)
            else:
                ...
                if not np.allclose(self.atoms.get_array('occupancies'), self.occupancy):
                    warnings.warn('Occupancies of atoms in group do not match group occupancy.')
        else:
            assert self.atoms.has('occupancies')
            self.occupancy = self.atoms.get_array('occupancies').mean()
...
def from_disorder_components(atoms_maj, atoms_min, tolerance=1e-2, symprec=1e-3, ratio=None, group_occupancies=None):
    """
    ...
    ordered_atoms = atoms_maj[[i for i in range(len(atoms_maj)) if i not in diff_idx]].copy()
