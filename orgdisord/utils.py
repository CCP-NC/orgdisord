...
def reload_as_molecular_crystal(
    images: List[Atoms], parallel: bool = True, cheap: bool = False, wrap: bool = False
):
    """
    ...
    if cheap:
        ...
        if parallel and ncores > 1:
            ...
            with Pool(ncores) as p:
                jobs = [
                    p.apply_async(unwrap_molecules, args=(atoms, wrap))
                    for atoms in images
                ]
            ...
            images_new = [
                unwrap_molecules(image, wrap_each_molecule=wrap) for image in images
            ]
...
def wrap_molecule(atoms: Atoms, max_dist: float = 0.5):
    """
...
def molecule_collide(
    atom: Atom, atoms: Atoms, vdw_scale: float = 1.0, tolerance: float = 0.0
):
    """
...
def get_unique_atoms(atoms: Atoms, sg: Spacegroup, symprec: float = 1e-3):
    ...
    if len(set(counts)) != 1:
        warnings.warn(
            "We have some groups of atoms that are of different lengths -- check!"
        )
