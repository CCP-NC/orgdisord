...
class OrderedfromDisordered:
    ...
    def get_supercell_configs(self,
        ...
        if ratio and not fix_ratio:
            self.logger.warn('Warning! ratio is specified but fix_ratio is False. Ignoring ratio.')
        ...
        if fix_ratio:
            self.logger.info(f'Only {len(all_configs)}/{n_configs} configs had the specified ratio of {self.ratio}')
        if return_configs:
    ...
    def _process_ratio(self, ratio):
        ...
        if ratio is None:
            ...
            ratio = [[g.occupancy for g in ass.disorder_groups] for ass in self.disordered_structure.disorder_assemblies]
    ...
    def _ratio_filter(self, config):
        ...
        ratio = self.ratio
        if len(ratio) > 1:
            raise NotImplementedError('ratio filtering not implemented for multiple assemblies')
    ...
    def get_supercell_config(self, config) -> Atoms:
        ...
        for icell, c in enumerate(select_configs(
                                        config,
                                        supercell=self.supercell,
                                        maxgroups=ndisordergroups)):
